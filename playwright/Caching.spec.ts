...
test.describe('Caching', () => {
    ...
    test('can view non-cached values', async ({ page }) => {
        ...
        expect(await cachedHeader.textContent()).toBe('Playwright testing world!')

        const expiringHeader = story.getByRole('heading', { name: /expiring/i })
        expect(await expiringHeader.textContent()).toBe('Playwright testing world!')
    })
    ...
    test('can view cached values', async ({ page }) => {
        ...
        const cachedHeader = story.getByRole('heading', { name: /cached/i })
        expect(await cachedHeader.textContent()).toBe('Playwright testing world! - cached')

        const expiringHeader = story.getByRole('heading', { name: /expiring/i })
        expect(await expiringHeader.textContent()).toBe('Playwright testing world! - cached')
    })
