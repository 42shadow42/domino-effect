...
test.describe('Splitting Dominos', () => {
	test('loads story', async ({ page }) => {
		await page.goto('?path=/story/domino-rules--splitting-dominos')

		// Expect a title "to contain" a substring.
		await expect(page).toHaveTitle(/Domino \/ Rules - Splitting Dominos/i)
	})
	...
	test('can view display value', async ({ page }) => {
		...
		const displayValue = story.getByRole('heading', {
			name: /display value/i,
		})

		expect(await displayValue.textContent()).toBe(
			'Playwright testing is so fun!',
		)
	})
})
