...
test.describe('Context', () => {
	...
	test('can edit contextual value', async ({ page }) => {
		...
		const input = story.getByRole('textbox', {
			name: /^contextual value$/i,
		})
	...
	test('can view contextual values', async ({ page }) => {
		...
		const contextualValueInput = story.getByRole('textbox', {
			name: /^contextual value$/i,
		})
		await contextualValueInput.fill('Contextual value')

		const nonContextualValueInput = story.getByRole('textbox', {
			name: /non contextual value/i,
		})
		...
		expect(await nonContextualValueInput.inputValue()).toBe(
		...
		expect(await contextualValueInput.inputValue()).toBe(
			'Contextual value 2',
		)
		expect(await nonContextualValueInput.inputValue()).toBe(
			'Non contextual value',
