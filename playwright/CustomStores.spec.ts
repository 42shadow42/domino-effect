...
test.describe('Custom Stores', () => {
	...
	test('are independent', async ({ page }) => {
        ...
        const localStoreInput = story.getByRole('textbox', { name: /local store/i })
		const localStoreHeading = story.getByRole('heading', { name: /local store/i })
        await localStoreInput.fill('Local Store')

        const customStoreInput = story.getByRole('textbox', { name: /custom store/i })
		const customStoreHeading = story.getByRole('heading', { name: /custom store/i })
        await customStoreInput.fill('Custom Store')

		const defaultStoreInput = story.getByRole('textbox', { name: /default store/i })
		const defaultStoreHeading = story.getByRole('heading', { name: /default store/i })
        await defaultStoreInput.fill('Global Store')

		expect(await defaultStoreHeading.textContent()).toBe('Global Store world!')
        expect(await customStoreHeading.textContent()).toBe('Custom Store world!')
